...
static TAutoConsoleVariable<float> CVarFilterActorVolume(
	TEXT("WorldTileCollection.FilterActorVolume"),
	0,
	TEXT("Filter Actor by volume when generating LOD."),
	ECVF_Default);
	...
	for (TSharedPtr<FWorldTileModel> TileModel : TilesToProcess)
	{
		...
		for (AActor* Actor : TileModel->GetLevelObject()->Actors)
		{
			if (Actor)
			{
				...
				// filter actor by aabb volume
				else if (FilterActorByVolume(Actor))
				{
					Actors.Add(Actor);
				}
			}
...
bool FWorldTileCollectionModel::FilterActorByVolume(const AActor* Actor)
{
	FVector Origin;
	FVector Box;
	Actor->GetActorBounds(false, Origin, Box);

	float Volume = Box.X * Box.Y * Box.Z * 8;
	static const auto CVar = IConsoleManager::Get().FindTConsoleVariableDataFloat(TEXT("WorldTileCollection.FilterActorVolume"));

	if (Volume >= CVar->GetValueOnAnyThread())
		return true;

	UE_LOG(LogTemp, Display, TEXT("---------- Filter actor: %s, volume: %.2f"), *Actor->GetName(), Volume);
	return false;
}
